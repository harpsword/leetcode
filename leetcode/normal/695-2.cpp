#include<iostream>
#include<vector>
#include<set>
// 将每个空格看做一个节点，每个节点最多有四个连接，问题转化为寻找最大的无向图连通分量
// 但是这个算法效率不高
// 更好的算法为递归算法求出每一个节点处的岛屿面积，取最大值。
using namespace std;

class Solution {
public:
    int maxAreaOfIsland(vector<vector<int>>& grid) {
        int l = grid.size();
        int r = grid[0].size();
        int node_n = l * r;

        set<int> visited;
        int count{0};
        int prev_visited_n{0};

        for (int i=0; i<l; i++){
            for (int j=0; j<r; j++){
                int node_id = i * r + j;
                if (grid[i][j] == 0)
                    visited.insert(node_id);
            }
        }
        
        while (visited.size()<node_n)
        {
            prev_visited_n = visited.size();
            int rooti{0}, rootj{0};
            bool find{false};
            for (int i=0; i<l; i++){
                for (int j=0; j<r; j++){
                    int node_id = i * r + j;
                    if (visited.find(node_id)==visited.end()){
                        visited.insert(node_id);
                        rooti = i;
                        rootj = j;
                        find = true;
                        break;
                    }
                }
                if (find) break;
            }
            dfs(grid, visited, rooti, rootj, l, r);

            if (visited.size()-prev_visited_n>count) 
                count = visited.size()-prev_visited_n;
        }
        return count;   
    }

    void dfs(vector<vector<int>>& grid, set<int>& visited, int i, int j, int l, int r){
        vector<int> x = {-1, 1, 0, 0};
        vector<int> y = {0, 0, -1, 1};
        int tmpi, tmpj;
        int new_node_id;
        for (int ii=0; ii < 4; ii++){
            tmpi = i + x[ii];
            tmpj = j + y[ii];
            new_node_id = tmpi * r + tmpj;
            if (tmpi>=0 && tmpi < l && tmpj >= 0 && tmpj<r){
                if (grid[tmpi][tmpj]==1 && visited.find(new_node_id) == visited.end()){
                    visited.insert(new_node_id);
                    dfs(grid, visited, tmpi, tmpj, l, r);
                }
            }

        }
    }

};


int main(){
    vector<vector<int>> m(4, vector<int>(5, 0));
    m[0][0] = 1;
    m[0][1] = 1;
    m[1][0] = 1;
    m[1][1] = 1;

    m[2][3] = 1;
    m[2][4] = 1;
    m[3][3] = 1;
    m[3][4] = 1;
    Solution s = Solution();
    cout << s.maxAreaOfIsland(m);
}